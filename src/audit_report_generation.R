...
visualize_tree <- function(t, ct, mydata, psi_metric){
  ...
  ggsave(filename=paste("output/TEMP_tree", t,".pdf", sep = ""), scale=1, width=12, height=6)
}
...
export_audit_report <- function(results_df, n_grp, psi_metric, ranking, partitioning, mydata){
  ...
  pdf("output/TEMP_compas_race_top_n_groups.pdf", height = 6, width = 12, onefile = FALSE) # 8.3 * 11.7
  
  # title
  title_g <-  textGrob("Audit report", gp=gpar(fontsize=18, fontface="bold"))
  ...
  tab_g <- tableGrob(out_tab, theme = ttheme_default(base_size=10.5, core=list(fg_params=list(hjust=0, x=0.05))),
  ...
  # merge table report with visualizations
  files_to_merge <- list.files(path = "output")
